...
This is a Python (Autobahn) variation of websocket servers designed to 
...
## Quick Set-Up
```
docker run -p 8080:8080 mtomasetti/python-autobahn_websocket-benchmark-server
```
That's it! The websocket server will automatically start on **port 8080**

## Dockerfile Set-Up
```
docker build . -t websocket_benchmark/python-autobahn
docker run -p 8080:8080 websocket_benchmark/python-autobahn
```
...
python3 python-autobahn_websocket-benchmark-server.py
```
